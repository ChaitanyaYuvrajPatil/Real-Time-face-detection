...
while True:
    ...
    for x,y,w,h in face:
        try:
            face_image = new_image[y:x+h,x:x+w]
        ...
        if label == 0:
            ...
            cv2.putText(new_image,"Mask",(x,y),cv2.FONT_HERSHEY_COMPLEX,0.8,(0,255,0),2)
        elif label ==0:
            cv2.rectangle(new_image, (x, y), (x + w, y + h), (0, 255, 0), 2)
            cv2.putText(new_image, "no_Mask", (x, y), cv2.FONT_HERSHEY_COMPLEX, 0.8, (0, 255, 0), 2)
        else:
            None
